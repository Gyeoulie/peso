<div class="container mx-auto py-8">
    ...
    <x-modal name="create-manager-modal" focusable>
        @if ($selectedBranch)
            <div class="w-full max-w-4xl px-6 py-6 items-center" x-data="{ agreeBox: @entangle('updateBox') }">
                <h2 class="text-lg font-medium text-gray-900">
                    {{ __('Are you sure you want to create this account?') }}
                </h2>
                <hr>
                ...
                <div class="mt-6 flex justify-between">
                    <x-secondary-button x-on:click="agreeBox = false; $dispatch('close-modal', 'create-manger-modal')">
                        {{ __('Cancel') }}
                    </x-secondary-button>

                    <x-green-button x-bind:disabled="!agreeBox" wire:loading.attr="disabled"
                        wire:click.prevent="updatePESO" class="ms-3" type="button">
                        {{ __('Create') }}
                        <div wire:loading.delay.long wire:target="createPESO" role="status">
                            <svg aria-hidden="true" class="w-4 h-4 text-gray-200 animate-spin fill-blue-600 ml-4"
                                viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
