<div class="bg-white shadow rounded-lg p-6">
    ...
    <div class="relative overflow-x-auto">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
            ...
            <tbody>
                @foreach ($certificate_type as $certtype)
                    <tr wire:key="{{ $certtype->cert_type_id }}" class="bg-white border-b hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="text-gray-500 font-medium text-lg uppercase"> {{ $certtype->cert_Code }}
                            </div>
                        </td>

                        <td class="px-6 py-4">
                            <div class="text-black font-bold text-lg uppercase"> {{ $certtype->cert_Name }}
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-base font-light uppercase"> {{ $certtype->created_at->format('F y, Y') }}
                            </div>

                        </td>
                        <td class="px-6 py-4">
                            <div class="text-base font-light uppercase"> {{ $certtype->updated_at->format('F y, Y') }}
                            </div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-row items-center justify-center gap-6">
                                ...
                                {{-- EDIT BUTTON --}}
                                <button type="button" wire:click.prevent='editCert({{ $certtype->cert_type_id }})'>
