<x-app-layout>
    ...
    <body>
                ...
                <div class="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4">
                    <div class="col-span-4 sm:col-span-3">
                        <div class="bg-white shadow rounded-lg p-6">
                            <div class="flex flex-col items-center">
                                @if ($data['companyInfo']->company_img)
                                    <img src="{{ asset('storage/' . $data['companyInfo']->company_img) }}"
                                        alt="Profile Image" class="w-32 h-32 bg-gray-300 rounded-full mb-4 shrink-0">
                                @else
                                ...
                                <h1 class="text-xl font-bold">{{ $data['companyInfo']->bussines_Name }}</h1>
                                <p class="text-gray-700">{{ $data['locType'][$data['companyInfo']->company_Type] }}</p>
                            ...
                            @if ($data['industryLine']->isNotEmpty())
                                <div class="flex flex-col mb-2">

                                    <span
                                        class="text-gray-700 uppercase font-bold tracking-wider mb-2">Industries</span>
                                    <ul>

                                        @foreach ($data['industryLine'] as $industryLine)
                                            <li class="mb-2">{{ $industryLine->industry->industry_Title }}</li>
                                        @endforeach
                    ...
                    <div class="col-span-4 sm:col-span-9">
                        <div class="bg-white shadow rounded-lg p-6">
                            ...
                            @if (!empty($data['companyInfo']->company_Desc))
                                <p class="text-gray-700 text-left">{{ $data['companyInfo']->company_Desc }}</p>
                            @else
                            ...
                            <div class="flex flex-row">
                                ...
                                <div class="flex flex-col mr-2 w-1/2">
                                    <h2 class="text-xl font-bold mt-6 mb-4">Contact Person</h2>

                                    <div class="mb-6">
                                        <div class="flex justify-between flex-col">
                                            <span
                                                class="text-gray-700 font-bold">{{ $data['companyInfo']->contact_Person }}</span>
                                            <p>
                                                <span
                                                    class="text-gray-700 font-semibold mr-2">{{ $data['companyInfo']->contact_Person_position }}</span>
        ...
        <x-modal name="adminDesc-modal" focusable>
            <div class="w-full max-w-4xl px-6 py-6 items-center border-b">
                <h2 class="text-lg font-medium text-gray-900">
                    {{ __('Edit Description') }}
                </h2>
                <hr>
                <form method="POST" action="{{ route('updateDescCompany') }}">
                    @csrf
                    <div class="flex flex-col mt-2">
                        ...
                        <textarea id="descModal" maxlength="400" rows="4"
                            ...
                            placeholder="Write your description here..." name="descPost" required>{{ $data['companyInfo']->company_Desc }}</textarea>
...
<script>
    function openDescModal() {
        ...
        window.dispatchEvent(new CustomEvent('open-modal', {
            detail: 'adminDesc-modal' // Assuming $name holds the modal name
        }));
    }
</script>
